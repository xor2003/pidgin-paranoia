 ...
 * less probable content will be rejected */
 	unsigned int keycount;		/* The Number of keys currently in production */
};
...
*	key generator (default), out of an entropy file or out of a character device The alice key
*	is stored at $HOME/.pranoia, the bob key in your home directory.
*	Currently there can only be one key at the moment generated. */
{
	...
	if(otp_conf_get_keycount(config) > 0) {
		g_print("Number of keys in generation: %i\n", otp_conf_get_keycount(config));
		return OTP_ERR_GENKEY_KEYGEN_IN_USE;
	}
	otp_conf_set_keycount(config, 1);
 ...
 * Sets random_msg_tail_max_len = DEFAULT_RNDLENMAX */
{
	...
	config->keycount = 0; /* Initialize with no keys in generation */
...
/* 	Sets the keycount, amount should only be 1 or -1 for increase or decrease
*	of the keycount in the config file */
{
	if(config == NULL) return OTP_ERR_INPUT;
	if((config->keycount + amount) < 0) return OTP_ERR_INPUT;
	config->keycount += amount;
	return OTP_OK;
}
